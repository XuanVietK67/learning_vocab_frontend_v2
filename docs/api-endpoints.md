...
| POST | `/v1/admin/vocabularies/quick/bulk` | JWT (admin) | Bulk quick-create phase 2. Body: `{ lemmas: string[] (1–500), language?, topics?: string[] }`. Creates one enrichment job per lemma under a shared `batchId`, skipping lemmas that already have a pending job or an existing system vocab. When `topics` is given (slugs must exist, else 400), every created draft is linked to them and any skipped lemma already in the catalog is tagged in place (tag-on-skip). Returns 202 `{ batchId, accepted, skipped }` (`batchId` is null when everything was skipped). |
