...
interface RevealBarProps {
  ...
  note?: ReactNode;
  /** Muted line under the strip (e.g. the practice "not counted" hint). */
  subtext?: ReactNode;
}
 ...
 */
export function RevealBar({ correct, answer, note, subtext }: RevealBarProps) {
  return (
    <div
      className={cn(
        "lr-pop flex flex-col gap-2 rounded-2xl px-4 py-3.5",
    ...
    >
      ...
      {subtext && (
        <div className="text-[12.5px] font-semibold opacity-75">{subtext}</div>
      )}
    </div>
  );
}
